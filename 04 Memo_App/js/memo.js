...
function(){
    ...
    if (typeof localStorage === "undefined") {
      window.alert("このブラウザはLocal Storage機能が実装させさせていません");
      return;
    } else {
      viewStorage();    //localStorageからデータ取得をテーブルへ表示
      saveLocalStorage();  //2.localStorageへの保存
      delLocalStorage(); //3.localStorage から１っけん削除
      allClearLocalStorage(); //4.LocalStorageからすべと削除
      selectTable();       //5  データ選択
    }
  }, false
);
